...
func init() {
	register(map[string][2]string{
		...
		"sweep.title":           {"Shaping rate sweep", "Şekillendirme hızı taraması"},
		"sweep.intro":           {"%d load tests, roughly two minutes each. The line is saturated throughout, so leave it alone until it finishes.", "%d adet yük testi, her biri kabaca iki dakika. Bu süre boyunca hat dolu olacak, bitene kadar dokunmayın."},
		"sweep.col.rate":        {"down/up", "indirme/yükleme"},
		"sweep.col.down":        {"got down", "gelen"},
		"sweep.col.up":          {"got up", "giden"},
		"sweep.col.grade":       {"grade", "not"},
		"sweep.col.bloat":       {"+ms", "+ms"},
		"sweep.unshaped":        {"unshaped", "şekillendirmesiz"},
		"sweep.best":            {"Best rate: %d/%d Mbit, latency under load +%.1f ms, at the cost of %.0f%% of download", "En iyi hız: %d/%d Mbit, yük altında gecikme +%.1f ms, indirmenin %%%.0f'ini vererek"},
		"sweep.none":            {"No rate kept latency under the threshold. The queue may not be in the modem - if the bottleneck is a switch or the LAN link itself, shaping here cannot reach it.", "Hiçbir hız gecikmeyi eşiğin altında tutamadı. Kuyruk modemde olmayabilir - darboğaz bir switch veya LAN bağlantısının kendisiyse buradan şekillendirmek oraya erişemez."},
		"sweep.notkept":         {"Shaping removed again. Re-run with --keep to install the winning rate.", "Şekillendirme kaldırıldı. Kazanan hızı kurmak için --keep ile tekrar çalıştırın."},
		"sweep.kept":            {"Rate left in force. It lives in kernel state only - run nabiz apply --only sqm to make it survive a reboot.", "Hız yürürlükte bırakıldı. Yalnızca çekirdek durumunda yaşar - yeniden başlatmayı atlatması için nabiz apply --only sqm çalıştırın."},
