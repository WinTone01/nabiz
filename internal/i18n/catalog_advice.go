...
func init() {
	register(map[string][2]string{
		...
		// --- queue / bufferbloat -------------------------------------------
		"adv.sqm.title": {"Cut bufferbloat with SQM (the real fix lives on the router)", "Bufferbloat'ı SQM ile kes (asıl çözüm router'da)"},
		"adv.sqm.why":   {"Latency rises by +%.0f ms under load (grade %s). The modem's queue is swelling; games and calls break even when the speed is fine.", "Yük altında gecikme +%.0f ms artıyor (not %s). Modemin kuyruğu şişiyor; hız yeterli olsa bile oyun ve görüşme bozulur."},
		"adv.sqm.s1":    {"On OpenWrt: install luci-app-sqm, choose cake + piece_of_cake.qos", "OpenWrt ise: luci-app-sqm kur, cake + piece_of_cake.qos seç"},
		"adv.sqm.s2":    {"Set the rates to about 92%% of what was measured: download ~%d Mbit, upload ~%d Mbit", "Hızları ölçülenin %%92'sine ayarla: indirme ~%d Mbit, yükleme ~%d Mbit"},
		"adv.sqm.s3":    {"If the router has no SQM, shape the upload direction on this machine:", "Router'da SQM yoksa bu makinede yükleme yönünü şekillendir:"},
		"adv.sqm.s4":    {"Then measure again", "Sonra tekrar ölç"},
		...
		"adv.sqm.risk":  {"Shaping costs a little peak throughput (~8%%); in exchange latency stays flat", "Şekillendirme tepe hızı biraz düşürür (~%%8); karşılığında gecikme sabitlenir"},
		...
		"adv.cake-gaming.title": {"Prioritise interactive traffic with cake diffserv", "cake diffserv ile etkileşimli trafiğe öncelik ver"},
